...
export const deleteJobUserService = async ({ jobId, userId }:IDeleteJobUserService) => {
  try {
    ...
    if (!userIsOwner) throw new AppError('Você não tem autorização para executar essa ação', 401);
